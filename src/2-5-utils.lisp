...
(declaim (reentrant boolean-diff))
(defgeneric boolean-diff (f v &optional manager)
  ...
  (:method ((f bdd-node) v &optional (manager (node-manager f)))
...
(declaim (reentrant compose))
(defun compose (f g v &optional (manager (node-manager f)))
  (declare (manager manager))
  (declare (bdd-node f))
  (declare (bdd-node g))
  (declare (type (and fixnum (integer 0)) v))

  (unless (and (eq manager (node-manager f))
               (eq manager (node-manager g)))
    (error 'cudd-manager-mismatch-error))
